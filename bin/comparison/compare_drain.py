#!/usr/bin/env python3
"""
Compare LogSim against Drain parser

Usage:
    python bin/comparison/compare_drain.py --dataset Apache --sample-size 1000
"""
...
def run_drain(dataset_name: str, sample_size: int = None):
    """Run Drain parser on specified dataset"""
    ...
    output_dir = Path(f'results/comparison/drain_{dataset_name.lower()}')
    ...
    try:
        ...
        print(f"ERROR: Drain parsing failed: {e}")
        return None
    ...
    # Save results
    output_file = output_dir / 'drain_results.json'
    with open(output_file, 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\n{'='*60}")
    print(f"Drain Results for {dataset_name}")
    print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(description='Run Drain parser comparison')
    ...
    results = run_drain(args.dataset, args.sample_size)
