#!/usr/bin/env python3
"""
Interactive CLI menu for logpress using rich library
"""
...
class InteractiveCLI:
    """Interactive CLI for logpress with rich UI"""
    ...
    def show_main_menu(self):
        ...
        console.print(Panel.fit(
            "[bold cyan]logpress - Log Compression System[/bold cyan]\n"
            ...
            if 0 <= idx < len(compressed_files):
                ...
                if not query:
                    ...
                    if key not in ('severity', 'ip') or not val:
                        ...
                        console.print()
                        import subprocess
                        cmd = ["python", "-m", "logpress", "query", "-c", str(selected_file), "--limit", "50"]
                        if key == 'severity':
                            cmd += ["--severity", val]
                        else:
                            cmd += ["--ip", val]
    ...
    def benchmark_comparison(self):
        ...
        console.print("[yellow]This will compare logpress against multiple compression tools:[/yellow]")
