...
This is the main Python package containing all LogSim functionality organized in **MCP (Model-Context-Protocol) architecture**.
...
### Models Layer (`models/`)
**Pure data structures** - No business logic, just dataclasses.
...
**Example**:
```python
from logsim.models import Token, LogTemplate
...
**Business logic** - Core algorithms and implementations.
...
**Usage**:
```python
from logsim.context.tokenization.tokenizer import LogTokenizer
...
**Usage**:
```python
from logsim.context.classification.semantic_types import SemanticTypeRecognizer
...
**Usage**:
```bash
python -m logsim compress -i input.log -o output.lsc
python -m logsim query -c output.lsc --severity ERROR
```
...
**Run tests**:
```bash
python -m pytest logsim/tests/ -v
```
...
from logsim.services import SemanticCompressor, QueryEngine
...
from logsim.protocols import EncoderProtocol, CompressorProtocol
```
...
```python
from logsim.services import SchemaEvaluator
...
python -m pytest logsim/tests/integration/ -v

# With coverage
python -m pytest logsim/tests/ --cov=logsim --cov-report=html
```
